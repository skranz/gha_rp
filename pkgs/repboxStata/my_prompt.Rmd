...
Ok, here we have a run_project.do that calls other do files. Besides run_project.do, only Overview_Aufbereiter.do was available, the other do files were missing. For some reason all regressions in Overview_Aufbereiter.do are twice the stata_run_cmd parcel. Normally, a do file that is called be a scheduler like run_project.do should not be called again separately. But perhaps that happened here.

Do you have any clue why we have all regressions from Overview_Aufbereiter.do twice with different runid. (Runids are far away from each other suggesting that the do file was run twice.)

Any helpful additional output you might need?
 
```Overview_Aufbereitet.do
clear all
set more off
set memory 100m
cap log close
